...
export const FateButton: React.FC<FateButtonProps> = ({
  ...
  const numericEnergy = typeof cosmicEnergy === 'string' ? 
    (cosmicEnergy === 'transcendent' ? 90 :
     cosmicEnergy === 'high' ? 70 :
     cosmicEnergy === 'medium' ? 50 :
     cosmicEnergy === 'low' ? 20 : 50) : cosmicEnergy;
  
  const finalEnergyLevel = numericEnergy;
  const isSpirituallyChallenged = disabled || (mercuryRetrograde && finalEnergyLevel < 30);
  
  // Convert numeric energy to CosmicEnergyLevel type
  const cosmicEnergyType: CosmicEnergyLevel = 
    finalEnergyLevel >= 80 ? 'transcendent' :
    finalEnergyLevel >= 60 ? 'high' :
    finalEnergyLevel >= 30 ? 'medium' : 'low';
  ...
  const baseClasses = 'relative inline-flex items-center justify-center font-medium rounded-lg transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 transform active:scale-95';
  ...
  return (
    ...
    >
      ...
      {/* Button content */}
      <span className={`${isLoading || isChanneling ? 'opacity-0' : 'opacity-100'} transition-opacity duration-200`}>
        {children}
      </span>
