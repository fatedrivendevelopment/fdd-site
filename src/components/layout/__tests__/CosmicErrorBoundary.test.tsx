  ...
  describe('Given a CosmicErrorBoundary component', () => {
    describe('When children render without errors', () => {
      test('Then it should render children normally', () => {
        ...
        expect(screen.queryByTestId('cosmic-error-boundary')).not.toBeInTheDocument();
      });
    ...
    describe('When children throw an error', () => {
      ...
      test('Then it should display cosmic error message', () => {
        ...
        expect(screen.getByRole('heading', { name: /cosmic disturbance detected/i })).toBeInTheDocument();
        // Check for any of the possible spiritual guidance messages
        const guidanceText = screen.getByText(/aura may need cleansing|spiritual guardians are protecting|Component energies are not harmonizing/i);
    ...
    describe('When interacting with error UI', () => {
      ...
      test('Then it should display error details', () => {
        ...
        expect(errorContent.textContent).toMatch(/(cosmic|spiritual|energy|aura|mystical)/i);
      });
    });
