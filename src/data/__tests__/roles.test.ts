...
describe('mysticalRoles data', () => {
  describe('mysticalRoles array', () => {
    ...
    test('should have valid aura colors', () => {
      ...
      const validAuraColors = ['red', 'orange', 'yellow', 'green', 'blue', 'indigo', 'violet', 'purple', 'gold', 'silver'];
    ...
    test('should have at least one responsibility per role', () => {
      ...
      mysticalRoles.forEach(role => {
        ...
        expect(role.responsibilities.every(r => typeof r === 'string' && r.length > 0)).toBe(true);
      });
    });
    ...
    test('should have at least one mystical tool per role', () => {
      ...
      mysticalRoles.forEach(role => {
        ...
        expect(role.mysticalTools.every(tool => typeof tool === 'string' && tool.length > 0)).toBe(true);
      });
    });
