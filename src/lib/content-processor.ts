 ...
 */
export async function processMarkdownContent(slug: string): Promise<MarkdownContent | null> {
  try {
 ...
 */
function inferCosmicEnergy(content: string): 'low' | 'medium' | 'high' | 'transcendent' {
  const energyKeywords = {
    transcendent: ['cosmic', 'transcendent', 'sacred', 'mystical', 'spiritual', 'enlightenment'],
    high: ['ritual', 'ceremony', 'meditation', 'wisdom', 'energy', 'manifestation'],
 ...
 */
function inferAuraColor(slug: string): 'red' | 'orange' | 'yellow' | 'green' | 'blue' | 'purple' | 'violet' | 'gold' | 'silver' {
  const colorMap: Record<string, 'red' | 'orange' | 'yellow' | 'green' | 'blue' | 'purple' | 'violet' | 'gold' | 'silver'> = {
    'manifesto': 'gold',
    'pdr': 'purple',
    'roles': 'blue',
    'rituals': 'violet',
...
function inferTags(content: string): string[] {
  const tagKeywords = [
    'development', 'spiritual', 'cosmic', 'ritual', 'ceremony',
    'meditation', 'wisdom', 'energy', 'manifestation', 'process',
    'methodology', 'practice', 'technique', 'sacred', 'mystical',
    'enlightenment', 'transcendent', 'divine', 'universal'
  ];
 ...
 */
function inferMysticalLevel(content: string): 'novice' | 'adept' | 'master' | 'cosmic-sage' {
  const complexity = {
    codeBlocks: (content.match(/```/g) || []).length / 2,
    headers: (content.match(/^#+/gm) || []).length,
    lists: (content.match(/^[\*\-\+]/gm) || []).length,
    length: content.length,
    mysticalTerms: ['cosmic', 'transcendent', 'sacred', 'mystical', 'spiritual', 'enlightenment']
 ...
 */
export async function searchDocuments(query: string): Promise<MarkdownContent[]> {
...
function calculateRelevanceScore(doc: MarkdownContent, query: string): number {
  ...
  // Tag matches
  score += doc.metadata.tags.filter(tag => 
    tag.toLowerCase().includes(query)
  ).length * 50;
  
  // Content matches (weighted by cosmic energy)
  const contentMatches = (doc.rawContent.toLowerCase().match(new RegExp(query, 'g')) || []).length;
  const energyMultiplier = { low: 1, medium: 2, high: 3, transcendent: 4 }[doc.metadata.cosmicEnergy];
